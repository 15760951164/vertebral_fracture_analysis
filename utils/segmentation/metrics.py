...
class TpFpFnMetricPerLabel(TpFpFnMetricBase):
    def __call__(self, prediction_sitk, groundtruth_sitk, labels):
        tp_fp_fn_list = self.calculate_tp_fp_fn_lists(
            prediction_sitk, groundtruth_sitk, labels)
...
class PrecisionMetric(TpFpFnMetricPerLabel):
    def evaluate_function(self, tp, fp, fn):
        return tp / (tp + fp) if (tp + fp) > 0 else np.nan


class RecallMetric(TpFpFnMetricPerLabel):
    def evaluate_function(self, tp, fp, fn):
