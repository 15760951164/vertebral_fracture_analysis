...
def resample(input_image,
    ...
    assert image_dim == transform_dim, 'image and transform dim must be equal, are ' + \
        str(image_dim) + ' and ' + str(transform_dim)
...
def resample_to_spacing(image, new_spacing=[1.0]*3, interpolator='nearest'):
    """
    ...
    new_size = [int(old_sp * old_si / new_sp) for old_sp, old_si,
                new_sp in zip(old_spacing, old_size, new_spacing)]
...
def copy_information_additional_dim(src, dst):
    ...
    src_direction = np.array(
        src.GetDirection(), np.float32).reshape((src_dim, src_dim))
...
def split_label_image(image, labels):
    splits_np = utils.np_image.split_label_image(
        utils.sitk_np.sitk_to_np_no_copy(image), labels)
    splits = []
...
def split_multi_label_image(image, labels):
    # collapse channel dimension (expects 'channels_first'; numpy and itk dimension order is reversed!)
    dim_to_split = -1
    size = list(image.GetSize())
    size[dim_to_split] = 0
    splits = []
...
def transform_np_output_to_input(output_image, output_spacing, channel_axis, input_image_size, input_image_spacing, input_image_origin, input_image_direction, transform, interpolator='linear', output_pixel_type=None):
    if channel_axis is not None:
        output_images = utils.np_image.split_by_axis(
            output_image, axis=channel_axis)
    else:
...
def hausdorff_distances(image_0, image_1, labels, multi_label=False):
    ...
    for label_image_0, label_image_1 in zip(label_images_0, label_images_1):
        ...
        average_hausdorff_distance_list.append(
            current_average_hausdorff_distance)
...
def surface_distance(label_image_0, label_image_1):
    ...
    try:
        ...
        reference_distance_map = sitk.SignedMaurerDistanceMap(
            label_image_1, squaredDistance=False, useImageSpacing=True)
        reference_distance_map_arr = sitk.GetArrayViewFromImage(
        ...
        segmented_distance_map = sitk.SignedMaurerDistanceMap(
            label_image_0, squaredDistance=False, useImageSpacing=True)
        segmented_distance_map_arr = sitk.GetArrayViewFromImage(
            segmented_distance_map)
        segmented_surface = sitk.LabelContour(label_image_0)
        segmented_surface_arr = sitk.GetArrayViewFromImage(segmented_surface)

        seg2ref_distances = np.abs(
            reference_distance_map_arr[segmented_surface_arr == 1])
        ref2seg_distances = np.abs(
            segmented_distance_map_arr[reference_surface_arr == 1])

        all_surface_distances = np.concatenate(
            [seg2ref_distances, ref2seg_distances])
